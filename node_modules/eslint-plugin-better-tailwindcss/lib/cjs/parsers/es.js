...
exports.isESArrowFunctionExpression = isESArrowFunctionExpression;
...
function getLiteralsByESVariableDeclarator(ctx, node, variables) {
    const literals = variables.reduce((literals, variable) => {
        ...
        if ((0, matchers_js_1.isVariableName)(variable)) {
            if (!(0, utils_js_1.matchesName)(variable, node.id.name)) {
                return literals;
            }
            literals.push(...getLiteralsByESExpression(ctx, [node.init]));
        }
        else if ((0, matchers_js_1.isVariableRegex)(variable)) {
            literals.push(...getLiteralsByESNodeAndRegex(ctx, node, variable));
        }
        else if ((0, matchers_js_1.isVariableMatchers)(variable)) {
            if (!(0, utils_js_1.matchesName)(variable[0], node.id.name)) {
                return literals;
            }
...
function getLiteralsByESCallExpression(ctx, node, callees) {
    const literals = callees.reduce((literals, callee) => {
        ...
        if ((0, matchers_js_1.isCalleeName)(callee)) {
            if (!(0, utils_js_1.matchesName)(callee, node.callee.name)) {
                return literals;
            }
            literals.push(...getLiteralsByESExpression(ctx, node.arguments));
        }
        else if ((0, matchers_js_1.isCalleeRegex)(callee)) {
            literals.push(...getLiteralsByESNodeAndRegex(ctx, node, callee));
        }
        else if ((0, matchers_js_1.isCalleeMatchers)(callee)) {
            if (!(0, utils_js_1.matchesName)(callee[0], node.callee.name)) {
                return literals;
            }
...
function getLiteralsByESNodeAndRegex(ctx, node, regex) {
    ...
    return (0, regex_js_1.getLiteralsByNodeAndRegex)(ctx, node, regex, {
...
function getLiteralByESTemplateElement(ctx, node) {
    ...
    const priorLiterals = findPriorLiterals(ctx, node);
    const content = (0, utils_js_1.getContent)(raw, quotes, braces);
    const whitespaces = (0, utils_js_1.getWhitespace)(content);
    const indentation = (0, utils_js_1.getIndentation)(line);
    const multilineQuotes = getMultilineQuotes(node);
    return {
...
function getESObjectPath(node) {
    ...
    if (isESStringLike(node) && isInsideObjectValue(node)) {
        const property = (0, matchers_js_1.findMatchingParentNodes)(node, [(node) => {
...
function isESArrowFunctionExpression(node) {
